...
void writeHisto(){
  ...
  for(unsigned int i=0; i<5; i=i+1){
    ...
    TString title="Pcharged"; title+=i;
    hPcharged[i]=new TH1F(title,title,120,0,120);
    TString title="E_ecal"; title+=i;
    hEecal[i]=new TH1F(title,title,120,0,120);
    TString title="E_hcal"; title+=i;
    hEhcal[i]=new TH1F(title,title,40,0,40);
    TString title="E_lep"; title+=i;
    hElep[i]=new TH1F(title,title,40,0,40);
    TString title="cos_thru"; title+=i;
    hcosthru[i]=new TH1F(title,title,40,0,40);
    TString title="cos_thet"; title+=i;
    hcosthet[i]=new TH1F(title,title,40,0,40);
  }
  ...
  for(unsigned ifile=0; ifile<4;ifile++){
    ...
    for(int ievent=0; ievent<nevents; ievent=ievent+1){
      ...
      hPcharged[ifile]->Fill(Pcharged);
      hEecal[ifile]->Fill(E_ecal);
      hEhcal[ifile]->Fill(E_hcal);
      hElep[ifile]->Fill(E_lep);
      hcosthru[ifile]->Fill(cos_thru);
      hcosthet[ifile]->Fill(cos_thet);
    }
  }
